...
impl FileGateway {
    pub(crate) fn new(store_location: String) -> Self {
        let rwlocks: Vec<_> =
        std::iter::repeat_with(|| RwLock::new(0))
            .take(NUMBER_OF_LOCKS)
            .collect();
        let locks = Arc::new(rwlocks);
        Self { store_location , locks}
    }

    pub async fn write_file(&self, key: String, data: String, create_new: bool) -> Result<(), String> {
    ...
    pub(crate) async fn get_files_with_prefix(&self, prefix: String) -> Result< Vec<Key>, StatusCode> {
    ...
    pub fn read_lock_key(&self, key: &String) -> Result<RwLockReadGuard<'_, i32>, PoisonError<RwLockReadGuard<'_, i32>>> {
    ...
    pub fn write_lock_key(&self, key: &String) -> Result<RwLockWriteGuard<'_, i32>, PoisonError<RwLockWriteGuard<'_, i32>>> {
        // Locks are in a bucket and each key locks a RwLock from that bucket.
        // For simplicity the lock bucket is based on the length of the key
        // other methods like hasing can also be used. 
        let index = key.len() % NUMBER_OF_LOCKS;
