    ...
    if json_body.is_array() {
        ...
        if check_for_repeated_key(&json_key_values) {
            ...
            let create_tasks: Vec<_>  = json_key_values
                                    .iter()
                                    .map(|val| tokio::spawn(
                                            create_or_update_key(State(state.clone()),val.clone(), create_new)
                                        )
                                    )
                                    .collect();
...
async fn create_or_update_key( State(state): State<Arc<ServerState>>,kv: KeyValue, create_new: bool) -> Result<String, String> {
    ...
    let write_res = state.file_gateway.write_file(kv.key.clone(), kv.value.clone(), create_new).await;
    match write_res {
        Ok(_) => {
            debug!("storing in cache from write {},{}",kv.key, kv.value.clone());
                state
                .cache
                .insert(kv.key.clone(), kv.value.clone()).await;
                Ok(kv.key)
        },
        Err(err) => Err(err)
    }
